...
def _common_switch_command(status: str, switch_ip: str, switch_port: int) -> ErrorCodes:
    ...
    on_off_str = 1 if status == __ON else 0
    cmd = f' curl \'http://10.42.0.244/set.cmd?user=admin+pass=12345678+cmd=setpower&p6{switch_port}={on_off_str}\'   -o /dev/null ' 
    # print(cmd)
    # print(tmp_file)
    # curl 'http://10.42.0.244/set.cmd?user=admin+pass=12345678+cmd=getpower'
    
    result = os.system(f"{cmd} 2>{tmp_file}")
    with open(tmp_file) as err:
        success_line = False
        for line in err:
            # m = re.match(r"\d+ {1}\d+ {1}\d+ {1}\d+ {1}\d+ {1}\d+ {1}\d+ {1}\d+ --:--:-- --:--:-- --:--:-- \d+", line)
            m = re.match(r"\d+\s+\d+\s+\d+\s+\d+\s+\d+\s+\d+\s+\d+\s+\d+\s+--:--:--\s+--:--:--\s+--:--:--\s+\d+", line)
            if m:
                success_line = True
